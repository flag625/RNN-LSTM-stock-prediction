...
"""
@author: lilianweng
地址：https://github.com/lilianweng/stock-rnn
"""
...
class LstmRNN(object):
    ...
    def build_graph(self):
        ...
        self.symbols = tf.placeholder(tf.int32, [None,1],
                                      name="stock_labels")
        # inputs.shape = (number of examples, number of inputs, dimension of each input).
        ...
        cell = tf.contrib.rnn.MultiRNNCell(
            [_creat_one_cell() for _ in range(self.num_layers)],
            state_is_tuple = True
        ) if self.num_layers > 1 else _creat_one_cell()
        ...
        self.preb = tf.matmul(last, ws) + bias
        ...
        self.preb_sum = tf.summary.histogram("preb", self.preb)

        #self.loss = -tf.reduce_sum(targets * tf.log(tf.clip_by_value(prediction, le-10, 1.0)))
        self.loss = tf.reduce_mean(tf.square(self.preb - self.targets), name="loss_mse")
        self.optim = tf.train.RMSPropOptimizer(self.learning_rate).minimize(self.loss, name="rmsprop_option")

        self.loss_sum = tf.summary.scalar("loss_mse", self.loss)
        self.learning_rate = tf.summary.scalar("learning_rate", self.learning_rate)
    ...
    def train(self, dataset_list, config):
        """
        :param dataset_list: StockDataSet
        :param config: tf.app.flags.FLAGS
        """
        assert  len(dataset_list) > 0
        self.merges_sum = tf.summary.merge_all()
        ...
        for epoch in range(config.max_epoch):
            epoch_step = 0
            learning_rate = config.init_learing_rate * (
                config.learning_rate_dacay ** max(float(epoch + 1 - config.init_epoch), 0.0)
            )
            ...
            for label_, d_ in enumerate(dataset_list):
                for batch_X, batch_y in d_.generate_one_epoch(config.batch_size):
                    ...
                    train_data_feed = {
                        ...
                        self.input_size: batch_X,
                        self.targets: batch_y,
                        self.symbols: batch_labels,
                    }
                    train_loss, _, train_merged_sum = self.sess.run(
                        [self.loss, self.optim, self.merges_sum], train_data_feed)
                    ...
                    if np.mod(global_step, len(dataset_list) * 100 / config.input_size) == 1:
                        test_loss, test_pred = self.sess.run([self.loss, self.preb], test_data_feed)
                        ...
                        for sample_sym, indices in sample_indices.iteritems():
                            ...
                            sample_prebs = test_pred[indices]
                            sample_truth = merged_test_y[indices]
                            self.plot_samples(sample_prebs, sample_truth, image_path, stock_sym=sample_sym)

                        self.save(global_step)
        final_preb, final_loss = self.sess.run([self.preb, self.loss], test_data_feed)
    ...
    def plot_samples(self, preds, targets, figname, stock_sym=None):
        ...
        plt.plot(days, preds, label='preb')
